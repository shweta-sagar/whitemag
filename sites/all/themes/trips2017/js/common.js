    ...
    Slick = (function() {
        ...
        function Slick(element, settings) {
            ...
            _.defaults = {
                ...
                arrows: true,
                asNavFor: null,
                prevArrow: '<button type="button" data-role="none" class="slick-prev" aria-label="Previous" tabindex="0" role="button"><i class="icon-angle-left"></i></button>',
                nextArrow: '<button type="button" data-role="none" class="slick-next" aria-label="Next" tabindex="0" role="button"><i class="icon-angle-right"></i></button>',
                autoplay: false,
                autoplaySpeed: 3000,
                centerMode: false,
                centerPadding: '50px',
...
$(document).on('click', '.filter_wm h2.showFilters', function (e) {
    var $this = $(this);
    if (!$this.hasClass('filters-collapsed')) {
        $('#filters').slideDown();
        $this.addClass('filters-collapsed');
    } else {
        $('#filters').slideUp();
        $this.removeClass('filters-collapsed');
    }
});

$('input#submitButton').click( function() {
	var radioValue = $("input[name='dateKey']:checked").val();
	 if (!radioValue) {
		alert('Please  select trip date.');
		return;
		} 
     if ($('.trip-price.currency').length>0) {	
			var currency=$('select[name=currency]').val();
			if(!currency) {
			alert('Please select currency.\nNon Indian have to make payment in USD');
			return;
			}
		    $("form[name='formSubmit"+radioValue+"'] input[name='trip_currency']").val(currency);	
			//alert(currency); return;
		}		
   $("form[name='formSubmit"+radioValue+"']").submit();
});

$('input:radio[name="dateKey"]').change(function() {
	//alert("dateKey");
if ($('.trip-price.currency').length>0) {	
	 $('.trip-price.tripPrice').hide(); 
	 if($('.trip-price.currency:visible').length === 0) {
	 $('.trip-price.currency').show("slow"); 
	 }
 } 
});

/* On trip date selection show trip currency dropdown */
$('input:radio[name="date1Key"]').change(function() {
	//alert("date1Key");
if ($('.trip-price.currency').length>0) {	
	 $('.trip-price.tripPrice').hide(); 
	 if($('.trip-price.currency:visible').length === 0) {
...
/* When Trip booking button is clicked*/
$('input#submitButton1').click( function() {
	var radioValue = $("input[name='date1Key']:checked").val();
	 if (!radioValue) {
		alert('Please select trip date.');
		return;
		}
		/* logic to validate if Trip Currency is selected */
		if ($('.trip-price.currency').length>0) {	
			var currency=$('select[name=currency1]').val();
			if(!currency) {
			alert('Please select currency.\nNon Indian have to make payment in USD');
			return;
			}
		    $("form[name='formSubmit1"+radioValue+"'] input[name='trip_currency']").val(currency);	
			//alert(currency); return;
		}	 
   $("form[name='formSubmit1"+radioValue+"']").submit();
});


/**
 Show Hide Trip Listing for fixed departure
*/
function hideTrips(val,val2)
{
var checkedCount=0;	
var hideDiv='.'+val+'Dep';
/* Check if mobile filters are selected */
	$checkMobileFilter=$(".filter_wm h2.showFilters");
	if ($checkMobileFilter.hasClass('filters-collapsed')) {
        $('#filters').slideUp(1000);
        $checkMobileFilter.removeClass('filters-collapsed');
    }

/* iterate through all checked trips */
$('#depCheckboxes input[type=checkbox]').each(function() {
	if ($(this).is(":checked")) {
	   checkedCount++;
		//alert(checkedCount);
		var tripClass=$(this).attr('value');
		//alert(tripClass);
       $("."+tripClass+'Dep').show();
   }else{
	  // alert(checkedCount+" no");
      var tripClass=$(this).attr('value');
	  //alert(tripClass);
	  $("."+tripClass+'Dep').hide();
   }
...
// Show the trip listing if nothing selected.
if (checkedCount == 0){ 
   $('.trip_box_slide .trip-container').show();
}
// If no trip show message.
if ($('.trip_box_slide .trip-container:visible').length === 0) {
	 if($('.trip_box_slide .result_content:visible').length === 0) {
            $(".trip_box_slide").append('<div class="result_content" style="text-align:left"><p>Sorry no trips were found to match your search.</p><p>It is recommended that you start with one filtering option (the one most important for you) and then narrow down using the other options to get to the desired result.</p><p>If you were using the search bar, the feature searches only the trip names. We suggest that you enter the trip name you are looking for. If you still do not get the desired result please try another trip name or find the trip by trip categories on the top menu.<br><br><br><br>Thank You !.</p></div>');
     }
 }else{
	$(".trip_box_slide .result_content").remove() ;
 }
...
$("#edit-submitted-trip-currency").change(function(){
	//alert("hi");
var currency=$('#edit-submitted-trip-currency').val();
$('.webform-component--trip-amount span.field-prefix').text(currency);
   // alert(currency);
});
if($("#edit-submitted-trip-currency").val()=="USD"){
	$('.webform-component--trip-amount span.field-prefix').text("USD");
}

$('.bookingBtn').click(function(){
   $('.inquiryBox').hide();	
   $('.bookingBox').fadeIn(4000);
}); 
$('.inquiryBtn').click(function(){
   $('.bookingBox').hide(); 	
   $('.inquiryBox').fadeIn(4000);	
  
});   

$('input#inquiryButton').click( function() {
	var radioValue = $("input[name='trip-date']:checked").val();
	 if (!radioValue) {
		alert('Please  select trip date.');
		return false;
		} 
...
$('input#inquiry1Button').click( function() {
	var radioValue = $("input[name='trip-date']:checked").val();
	 if (!radioValue) {
		alert('Please  select trip date.');
		return false;
